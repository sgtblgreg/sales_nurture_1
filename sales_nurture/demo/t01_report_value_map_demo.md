  ...
  <div class="kpiCard">
    ...
    <details class="sfDetails">
      ...
      <div class="sfDetailsBody">
        ...
        <ul>
          <li><b>GeoMatch</b> — доля источников, релевантных географии/рынку (локальные регуляторы, локальные кейсы, локальные цены/поведение).</li>
          <li><b>Freshness</b> — доля источников в окне актуальности (например, 12–18 месяцев) + штраф за устаревшие “ядра” корпуса.</li>
          <li><b>Trust</b> — доля независимых/проверяемых источников (A/B) и штраф за C (маркетинговые перепечатки, неподтверждённые claims).</li>
          <li><b>Diversity</b> — разнообразие доменов/типов + штраф за “инфо‑пузырь”.</li>
        </ul>
        <div class="expertOnly">
          <div style="margin-top:10px;"><b>Что такое “инфо‑пузырь” (в нашем смысле):</b> часть корпуса, где много повторов и мало независимых подтверждений (одни и те же тезисы циркулируют между близкими источниками/PR‑публикациями). Мы ловим это через:</div>
          <ul>
            <li>концентрацию доменов (top‑N доменов дают непропорциональную долю);</li>
            <li>дубли/перепечатки (одинаковые тезисы без новых данных);</li>
            <li>низкую долю первичных/проверяемых evidence (данные/исследования/регуляторы/аудит‑кейсы).</li>
          </ul>
          <div>Итого: даже при большом количестве материалов корпус может быть “шумным” — это напрямую снижает защищаемость downstream‑выводов.</div>
        </div>
      </div>
    </details>
  </div>
...
<div class="chartCard">
  ...
  <div class="chartNote">Итого: mix помогает понять, насколько downstream‑выводы “опираются на реальность” и где могут быть провалы в trust/diversity.</div>
</div>
...
<div class="vizStack">
  <div class="chartCard">
    <div class="chartTitle">Блок 3 — Каталоги: доверие → частота + карта (<span data-sf-t01-catalog-type-label>—</span> • <span data-sf-t01-catalog-count>—</span>)</div>
    <div class="chartNote">Переключай вкладки: сегменты / потребности / функц. решения / каналы / sales BP / триггеры. Клик по строке → раскрыть варианты и источники; кнопка «Карточка» → агрегированные метрики.</div>

    <div class="sfModalTabs" data-sf-t01-catalog-tabs></div>
  ...
  <div class="chartCard">
    <div class="chartTitle">Карта: <span data-sf-t01-catalog-type-label>—</span> • частота × доверие (<span data-sf-t01-catalog-viz-count>—</span>)</div>
